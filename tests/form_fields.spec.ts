...
import { formBodyNormalizers } from '../src/utils.ts'
...
test.group('Form Fields Parser', () => {
  ...
  test('convert empty string to null', ({ assert }) => {
    const formFields = new FormFields(formBodyNormalizers.convertToNull)
  ...
  test('trim whitespaces', ({ assert }) => {
    const formFields = new FormFields(formBodyNormalizers.trimWhitespacesAndConvertToNull)
    formFields.add('username', '     hello')
    assert.deepEqual(formFields.get(), { username: 'hello' })
  })

  test('trim whitespaces and convert to null', ({ assert }) => {
    const formFields = new FormFields(formBodyNormalizers.trimWhitespacesAndConvertToNull)
    formFields.add('username', '     ')
    assert.deepEqual(formFields.get(), { username: null })
  })
