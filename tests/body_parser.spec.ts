...
test.group("BodyParser Middleware | form data", () => {
  ...
  test("abort when multipart body is invalid", async ({ assert, cleanup }) => {
    ...
    await new Promise<void>((resolve) =>
      server.listen(3333, "localhost", () => resolve()),
    );
  ...
  test("abort when multipart body is invalid newline characters", async ({
    assert,
    cleanup,
  }) => {
    const server = createServer(async (req, res) => {
...
test.group("BodyParser Middleware | multipart", () => {
  ...
  test("do not process request when autoProcess is false", async ({
    assert,
  }) => {
    assert.plan(2);
  ...
  test("do not process request when processManually static route matches", async ({
    assert,
  }) => {
    assert.plan(2);
  ...
  test("do not process request when processManually has dynamic route", async ({
    assert,
  }) => {
    assert.plan(2);
  ...
  test("do not process request when autoProcess route does not match", async ({
    assert,
  }) => {
    assert.plan(2);
  ...
  test("process request when autoProcess route does matches", async ({
    assert,
  }) => {
    const server = createServer(async (req, res) => {
      const request = new RequestFactory().merge({ req, res }).create();
  ...
  test("detect file ext and mime type using magic number", async ({
    assert,
  }) => {
    const server = createServer(async (req, res) => {
      const request = new RequestFactory().merge({ req, res }).create();
  ...
  test("validate file when access via request.file method", async ({
    assert,
  }) => {
    const server = createServer(async (req, res) => {
      const request = new RequestFactory().merge({ req, res }).create();
  ...
  test("validate array of files when access via request.file method", async ({
    assert,
  }) => {
    const server = createServer(async (req, res) => {
      const request = new RequestFactory().merge({ req, res }).create();
      ...
      await middleware.handle(ctx, async () => {
        res.writeHead(200, { "content-type": "application/json" });
        const pkgFiles = ctx.request
          .files("package", { size: 10 })
          .map((pkgFile) => {
            return {
              tmpPath: pkgFile.tmpPath!,
              size: pkgFile.size,
  ...
  test("raise error when destination file already exists", async ({
    assert,
    fs,
  }) => {
    const server = createServer(async (req, res) => {
  ...
  test("validate file extension and file size seperately", async ({
    assert,
  }) => {
    const server = createServer(async (req, res) => {
      const request = new RequestFactory().merge({ req, res }).create();
  ...
  test("updating sizeLimit multiple times must not be allowed", async ({
    assert,
  }) => {
    const server = createServer(async (req, res) => {
      const request = new RequestFactory().merge({ req, res }).create();
  ...
  test("updating allowedExtensions multiple times must not be allowed", async ({
    assert,
  }) => {
    const server = createServer(async (req, res) => {
      const request = new RequestFactory().merge({ req, res }).create();
    ...
    assert.equal(
      text,
      "Cannot update allowed extension names after file has been validated",
    );
  });
